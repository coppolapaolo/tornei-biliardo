# ============================================================================
# PHASE 1 PERMISSION SYSTEM UPDATE - Add at the top of utils.py
# ============================================================================
...
# Update existing decorators to use the new permission system
# These replace/extend the existing decorators in utils.py


def admin_required(f):
    """Updated admin_required decorator using new permission system"""
    return RoleRequirement.admin_required(f)


def director_or_admin_required(f):
    """Updated director_or_admin_required decorator"""
    return RoleRequirement.director_or_admin_required(f)


def tournament_manager_required(tournament_id_getter):
    """Updated tournament_manager_required decorator"""
    return RoleRequirement.tournament_manager_required(tournament_id_getter)


def prova_manager_required(f):
    """Updated prova_manager_required decorator"""
...
def match_manager_required(f):
    """Updated match_manager_required decorator"""
            ...
            for player_id in [match.player1_id, match.player2_id]:
                if player_id not in players_stats:
                    ...
                    players_stats[player_id] = {
                        "matches_won": 0,
                        "point_diff": 0,
                        "initial_order": inscription.initial_order
                        if inscription
                        else 999,
                    }
