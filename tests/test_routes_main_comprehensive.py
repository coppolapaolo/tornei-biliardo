...
class TestMainRoutes:
    ...
    def test_index_authenticated_user_redirects(self, client, app_context):
        ...
        with patch('routes.main.current_user') as mock_user, \
             patch('routes.main.redirect') as mock_redirect, \
             patch('routes.main.url_for') as mock_url_for:
    ...
    @patch('routes.main.Tournament')
    @patch('routes.main.current_user')
    def test_index_with_active_tournaments(self, mock_user, mock_tournament, mock_prova, mock_classification, mock_render):
        ...
        mock_tournament.query.filter_by.return_value.order_by.return_value.all.return_value = mock_tournaments
        
        # Mock upcoming provas
        mock_upcoming_provas = [Mock(), Mock()]
        mock_prova.query.filter.return_value.order_by.return_value.limit.return_value.all.return_value = mock_upcoming_provas
        
        # Mock top classifications
        mock_top_classifications = [Mock(), Mock(), Mock()]
        mock_classification.query.filter.return_value.order_by.return_value.limit.return_value.all.return_value = mock_top_classifications
    ...
    @patch('routes.main.Config')
    def test_reset_database_confirm_wrong_password(self, mock_config, mock_request, mock_flash, mock_url_for, mock_redirect):
    ...
    @patch('routes.main.request')
    @patch('routes.main.Config')
    def test_reset_database_confirm_success(self, mock_config, mock_request, mock_user, mock_logout, mock_db, mock_reset_enhanced, mock_flash, mock_url_for, mock_redirect):
        ...
        assert any("Sei stato disconnesso automaticamente" in msg for msg in flash_calls)
    ...
    @patch('routes.main.request')
    @patch('routes.main.Config')
    def test_reset_database_confirm_exception(self, mock_config, mock_request, mock_user, mock_db, mock_flash, mock_url_for, mock_redirect):
    ...
    @patch('routes.main.Config')
    def test_quick_login_user_not_found(self, mock_config, mock_user_class, mock_flash, mock_url_for, mock_redirect):
    ...
    @patch('routes.main.User')
    @patch('routes.main.Config')
    def test_quick_login_success(self, mock_config, mock_user_class, mock_login_user, mock_flash, mock_url_for, mock_redirect):
    ...
    @patch('routes.main.User')
    @patch('routes.main.Config')
    def test_quick_login_user_query_called_correctly(self, mock_config, mock_user_class, mock_flash, mock_url_for, mock_redirect):
