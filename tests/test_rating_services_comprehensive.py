...
class TestRatingService:
    ...
    def test_get_user_rating_profile(self, mock_category_class, mock_rating_class):
        ...
        mock_category_class.get_user_current_category.return_value = mock_current_category
        ...
        with patch.object(RatingService, 'get_player_effective_category') as mock_effective:
            mock_effective.return_value = CategoryLevel.B
            mock_history = [Mock(), Mock()]
            mock_category_class.query.filter_by.return_value.order_by.return_value.all.return_value = mock_history
    ...
    def test_get_user_all_ratings(self, mock_rating_class, mock_rating_system):
        ...
        mock_rating_system.__iter__ = Mock(return_value=iter([
            Mock(value='FARGO'),
            Mock(value='ELO')
        ]))
    ...
    def test_verify_rating_success(self, mock_rating_class, mock_db):
        ...
        result = RatingService.verify_rating(rating_id=456, verified_by_id=789, verified=True)
    ...
    @patch('models.rating.services.PlayerRating')
    @patch('models.rating.services.PlayerCategory')
    def test_get_management_overview(self, mock_category_class, mock_rating_class, mock_db):
        """Test get_management_overview method."""
        mock_unverified = [Mock(), Mock(), Mock()]
        mock_rating_class.query.filter_by.return_value.order_by.return_value.all.return_value = mock_unverified
        
        mock_users_without_cats = [(123,), (456,)]
        mock_db.session.query.return_value.outerjoin.return_value.filter.return_value.distinct.return_value.all.return_value = mock_users_without_cats
        
        mock_recent = [Mock() for _ in range(5)]
        mock_rating_class.query.order_by.return_value.limit.return_value.all.return_value = mock_recent
    ...
    def test_get_system_statistics(self, mock_datetime, mock_category_level, mock_rating_system, mock_rating_class, mock_category_class, mock_handicap_rule):
        ...
        mock_rating_class.query.filter_by.return_value.all.return_value = [mock_fargo_rating]
    ...
    def test_assign_player_category(self, mock_category_class, mock_db):
        ...
        mock_existing = Mock()
        mock_category_class.query.filter_by.return_value.all.return_value = [mock_existing]
        
        with patch('models.rating.services.PlayerCategory') as mock_category_constructor:
    ...
    @patch('models.rating.services.PlayerRating')
    @patch('models.rating.services.PlayerCategory')
    def test_get_player_effective_category_assigned(self, mock_category_class, mock_rating_class):
        ...
        mock_category_class.get_user_current_category.return_value = mock_assigned_category
    ...
    @patch('models.rating.services.PlayerRating')
    @patch('models.rating.services.PlayerCategory')
    def test_get_player_effective_category_from_rating(self, mock_category_class, mock_rating_class):
    ...
    @patch('models.rating.services.db')
    @patch('models.rating.services.HandicapRule')
    def test_calculate_match_handicap_with_rule(self, mock_rule_class, mock_db, mock_handicap_service):
        ...
        mock_handicap_service._calculate_category_handicap.return_value = mock_category_result
        
        result = RatingService.calculate_match_handicap(123, 456, handicap_rule_id=789)
        
        assert result == mock_category_result
        mock_handicap_service._calculate_category_handicap.assert_called_once_with(123, 456, mock_rule)
...
class TestHandicapService:
    ...
    @patch('models.rating.services.HandicapRule')
    def test_create_handicap_rule(self, mock_rule_class, mock_rating_rule_class, mock_category_rule_class, mock_db):
        """Test create_handicap_rule method."""
        mock_rule = Mock()
        mock_rule.id = 123
        mock_rule_class.return_value = mock_rule
        
        category_rules = [{"higher_category": "A", "lower_category": "B", "handicap_value": 1}]
        rating_rules = [{"rating_system": "FARGO", "rating_difference_threshold": 50, "handicap_per_point": 100}]
    ...
    @patch('models.rating.services.PlayerCategory')
    @patch('models.rating.services.db')
    def test_auto_assign_categories_from_ratings(self, mock_db, mock_category_class, mock_rating_class, mock_assign):
        """Test auto_assign_categories_from_ratings method."""
        # Mock users with ratings
        mock_db.session.query.return_value.distinct.return_value.all.return_value = [(123,), (456,)]
        
        # Mock existing categories
        mock_category_class.get_user_current_category.side_effect = [None, Mock()]  # First user has no category, second has category
        
        # Mock rating lookup
        mock_rating = Mock()
        mock_rating.rating_value = 600
        mock_rating.get_category_equivalent.return_value = CategoryLevel.A
        mock_rating_class.get_user_rating.side_effect = [mock_rating, None, None]  # Only FARGO rating for first user
