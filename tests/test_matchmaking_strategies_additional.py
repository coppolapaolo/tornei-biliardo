...
class TestDirectEliminationStrategy:
    ...
    def test_create_first_round_even_players(self, sample_prova, db_session):
        ...
        players = []
        for i in range(4):
            user = User(username=f"direct_player{i}", email=f"direct{i}@test.com", role="player")
        ...
        for player in players:
            inscription = Inscription(prova_id=sample_prova.id, user_id=player.id, is_withdrawn=False)
    ...
    def test_create_round_matches_subsequent_round(self, sample_prova):
        ...
        with patch.object(strategy, '_get_round_winners') as mock_winners, \
             patch.object(strategy, '_create_match') as mock_create:
...
class TestDoubleKnockoutStrategy:
    ...
    def test_create_round_matches_first_round(self, sample_prova_double):
        ...
        with patch.object(strategy, 'get_active_players') as mock_players, \
             patch.object(strategy, '_create_winner_bracket_matches') as mock_winner_matches:
    ...
    def test_create_round_matches_subsequent_round(self, sample_prova_double):
        ...
        with patch.object(strategy, '_create_winner_bracket_matches') as mock_winner, \
             patch.object(strategy, '_create_loser_bracket_matches') as mock_loser:
...
class TestRandomAntiRematchStrategy:
    ...
    def test_create_round_matches_basic(self, sample_prova_random):
        ...
        with patch.object(strategy, 'get_active_players') as mock_players, \
             patch.object(strategy, '_get_previous_encounters') as mock_encounters, \
             patch.object(strategy, '_create_optimal_pairings') as mock_pairings:
...
class TestRoundRobinStrategy:
    ...
    def test_create_round_matches_robin(self, sample_prova_robin):
        ...
        with patch.object(strategy, 'get_active_players') as mock_players, \
             patch.object(strategy, '_get_round_pairings') as mock_pairings:
