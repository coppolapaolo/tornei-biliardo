...
class TestUnlockEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher_metrics = patch(
            "models.kpi.user_metrics.UserMetricService.get_metric"
        )
    ...
    def test_check_eligibility_disabled(self):
        ...
        self.mock_db_session.get.side_effect = lambda model, id: (
            config if model == FeatureConfig else None
        )
    ...
    def test_rule_level_requirement(self):
        ...
        config = FeatureConfig(
            code="level_feat", is_active=True, rules=json.dumps(rules)
        )
