...
class TestIndividualMatchServicesTransactionMigrationPhase2:
    ...
    def test_start_match_transaction_behavior(self, app, test_match_phase2):
        ...
        with app.app_context():
            ...
            # Act: start match
            IndividualMatchService.start_match(individual_match.id)
    ...
    def test_submit_rack_result_transaction_behavior(self, app, test_match_phase2):
        ...
        with app.app_context():
            ...
            # Act: submit rack result (modify existing rack)
            ...
            # Assert: rack was updated and committed
            # Verify changes exist in database (transaction was committed)
            db_rack = db.session.get(IndividualRack, rack.id)
            assert db_rack is not None
            assert db_rack.winner_id == individual_match.player2_id
            assert db_rack.notes == "Corrected winner"
    ...
    def test_complete_match_transaction_behavior(self, app, test_match_phase2):
        ...
        with app.app_context():
            ...
            # Add enough racks to complete the match (distance = 7, so need 7 wins)
            winner_id = individual_match.player1_id
            for i in range(7):
            ...
            # Act: complete match explicitly (should already be completed by add_rack_result)
            # But we test the explicit complete_match call
    ...
    def test_cancel_proposal_transaction_behavior(self, app, test_users_phase1):
        """
    ...
    def test_invite_player_to_match_transaction_behavior(self, app, test_users_phase1):
        """
    ...
    def test_respond_to_invitation_transaction_behavior(self, app, test_users_phase1):
        """
