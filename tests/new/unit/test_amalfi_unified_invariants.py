...
class TestAmalfiUnifiedInvariants:
    ...
    def create_preview_data(self, pairings_data: List[dict]) -> dict:
        ...
        return {
            ...
            "stats": {
                ...
                "trio_matches": sum(1 for m in pairings_data if m.get("type") == "trio"),
            },
            "salto": 0
        }
    ...
    def test_invariant_all_players_assigned_even(self, adapter, num_players):
        ...
        assert assigned_players == expected_players, f"Missing or duplicate players. Expected: {expected_players}, Got: {assigned_players}"
    ...
    def test_invariant_no_self_pairing(self, adapter):
        ...
        for pairing in pairings:
            if not pairing.is_bye and len(pairing.players) >= 2:
                assert pairing.players[0] != pairing.players[1], f"Self-pairing detected: {pairing.players}"
    ...
    def test_invariant_pairing_quality_bounds(self, adapter):
        ...
        pairings_data = [
            {"player1": Mock(id=1), "player2": Mock(id=2), "type": "normal"},
            {"player1": Mock(id=3), "player2": Mock(id=4), "type": "trio"},  # Lower quality
        ...
        for pairing in pairings:
            assert 0.0 <= pairing.pairing_quality <= 1.0, f"Invalid quality: {pairing.pairing_quality}"
    ...
    def test_invariant_round_number_consistency(self, adapter, round_num):
        ...
        for pairing in pairings:
            assert pairing.round_number == round_num, f"Expected round {round_num}, got {pairing.round_number}"
    ...
    def test_invariant_valid_pairing_structure(self, adapter):
        ...
        for pairing in pairings:
            ...
            if pairing.is_bye:
                assert len(pairing.players) == 1, f"Bye pairing must have 1 player: {pairing.players}"
            
            # Invariant: Normal pairings must have exactly 2 players
            else:
                assert len(pairing.players) == 2, f"Normal pairing must have 2 players: {pairing.players}"
    ...
    def test_invariant_validation_minimum_players(self, adapter):
        ...
        assert any("players" in error.lower() for error in result.errors), "Error should mention player count"
    ...
    def test_invariant_validation_success_sufficient_players(self, adapter):
        ...
        assert result.ok, f"Validation should succeed with sufficient players: {result.errors}"
    ...
    def test_invariant_deterministic_with_seed(self, adapter):
        ...
        for p1, p2 in zip(pairings1, pairings2):
            assert p1.players == p2.players, f"Deterministic failure: {p1.players} != {p2.players}"
    ...
    def test_invariant_strategy_metadata(self, adapter):
        ...
        assert adapter.name.lower() == "amalfi"
    ...
    def test_invariant_anti_rematch_different_rounds(self, adapter):
        ...
        if len(pairings_r1) == len(pairings_r2) == 1:
            ...
            assert p1.players != p2.players, "Anti-rematch should produce different pairings"