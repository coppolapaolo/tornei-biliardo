...
class TestAmalfiUnifiedPerformance:
    ...
    def create_large_preview_data(self, num_players: int) -> dict:
        ...
        if num_players % 2 == 1:
            matches.append({"player1": Mock(id=num_players), "type": "bye"})
    ...
    def test_small_tournament_performance(self, adapter, num_players):
        ...
        assert (
            execution_time < 2.0
        ), f"Preview took {execution_time:.3f}s for {num_players} players (>2s limit)"
    ...
    def test_large_tournament_performance(self, adapter, num_players):
        ...
        ), f"Preview took {execution_time:.3f}s for {num_players} players (>{max_time}s limit)"
    ...
    def test_context_setup_overhead(self, adapter):
        ...
        overhead_ratio = (
            time_with_context / time_without_context if time_without_context > 0 else 1
        )
        assert (
            overhead_ratio < 1.1
        ), f"Context setup adds {(overhead_ratio-1)*100:.1f}% overhead (>10% limit)"
    ...
    def test_multiple_rounds_performance(self, adapter):
        ...
        assert (
            avg_time_per_round < 0.5
        ), f"Average {avg_time_per_round:.3f}s per round (>0.5s limit)"

        print(
            f"✓ 5 rounds total: {total_time:.3f}s (avg: {avg_time_per_round:.3f}s/round)"
        )
    ...
    def test_memory_efficiency(self, adapter):
        ...
        assert (
            ref_growth < 20
        ), f"Reference count grew by {ref_growth} (potential memory leak)"
    ...
    def test_validation_performance(self, adapter):
        ...
        _, validation_time = self.measure_execution_time(adapter.validate, gara)

        # Validation should be very fast (< 0.1s even for large tournaments)
        assert (
            validation_time < 0.1
        ), f"Validation took {validation_time:.3f}s (>0.1s limit)"
    ...
    def test_performance_regression(self, adapter):
        ...
        for num_players, max_time in test_cases:
            ...
            assert (
                execution_time < max_time
            ), f"{num_players} players took {execution_time:.3f}s (>{max_time}s baseline)"
            print(
                f"✓ {num_players} players: {execution_time:.3f}s (baseline: <{max_time}s)"
            )
    ...
    def test_metrics_collection_performance(self, adapter):
        ...
        print(
            f"✓ Metrics collection: {execution_time:.3f}s, {metrics.pairings_generated} pairings"
        )
