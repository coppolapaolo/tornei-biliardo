...
class TestAmalfiCompleteWorkflow:
    ...
    def test_complete_amalfi_gara_workflow_director(self, director_user, players, db_session):
    ...
    def _test_complete_amalfi_workflow(self, creator: User, players: List[User], db_session):
        ...
        for i, player in enumerate(players):
            ...
            assert inscription.is_waitlist is False  # Gara has max 10, so first 8 should not be waitlisted
        ...
        total_matches, normal_matches, bye_matches, trio_matches = GaraService.create_amalfi_round(
            gara.id, 2
        )
        ...
        round3_matches = Match.query.filter_by(
        ...
        completed_matches = [m for m in all_matches if m.status == MatchStatus.COMPLETED.value]
    ...
    def _verify_classification_ordering(self, classification: List):
        ...
        for i, player_data in enumerate(classification):
            ...
            if hasattr(player_data, 'wins'):
                ...
                diff = player_data.rack_difference if hasattr(player_data, 'rack_difference') else 0
            else:
            ...
            if prev_wins is not None:
                ...
                assert wins <= prev_wins, f"Position {i}: wins ordering violation (prev: {prev_wins}, current: {wins})"
                
                if wins == prev_wins and prev_diff is not None:
                    assert diff <= prev_diff, f"Position {i}: rack difference ordering violation (prev: {prev_diff}, current: {diff})"
    ...
    def test_amalfi_anti_rematch_behavior(self, director_user, players, db_session):
        ...
        for round_num in range(1, 4):  # Test first 3 rounds
            if round_num == 1:
                ...
                prev_matches = Match.query.filter_by(gara_id=gara.id, round_number=round_num-1).all()
        ...
        print(f"✅ Anti-rematch test passed: {total_unique_pairings} unique pairings, {immediate_rematches} immediate rematches")
