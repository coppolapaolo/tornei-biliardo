...
def test_player_venue_manager_request_from_venues_list_page(app, client):
    ...
    with app.app_context():
        ...
        player = User(
            username="test_player_ui", email="player_ui@test.com", role="player"
        )
        ...
        test_venue = LocationService.create_billiard_hall(
            name="UI Test Venue List", address="123 UI List Street", city="UI List City"
        )
...
def test_director_venue_manager_request_from_venue_detail_page(app, client):
    ...
    with app.app_context():
        ...
        # Create test users
        admin = User(
            username="test_admin_detail", email="admin_detail@test.com", role="admin"
        )
        admin.set_password("password")
        db.session.add(admin)

        director = User(
            username="test_director_detail",
            email="director_detail@test.com",
            role="director",
        )
...
def test_venues_list_page_request_path(app, client):
    ...
    with app.app_context():
        ...
        # Create test users
        admin = User(
            username="test_admin_list_path",
            email="admin_list_path@test.com",
            role="admin",
        )
        admin.set_password("password")
        db.session.add(admin)
        ...
        player = User(
            username="test_player_list_path",
            email="player_list_path@test.com",
            role="player",
        )
        player.set_password("password")
        db.session.add(player)
        ...
        request = VenueManagerRequest.query.filter_by(
            user_id=player.id, venue_id=venue.id
        ).first()
...
def test_venue_detail_page_request_path(app, client):
    ...
    with app.app_context():
        ...
        # Create test users
        admin = User(
            username="test_admin_detail_path",
            email="admin_detail_path@test.com",
            role="admin",
        )
        admin.set_password("password")
        db.session.add(admin)
        ...
        director = User(
            username="test_director_detail_path",
            email="director_detail_path@test.com",
            role="director",
        )
        director.set_password("password")
        db.session.add(director)
        ...
        request = VenueManagerRequest.query.filter_by(
            user_id=director.id, venue_id=venue.id
        ).first()
        assert request is not None
        assert request.status == "pending"
...
def test_duplicate_request_handling_through_ui(app, client):
    ...
    with app.app_context():
        ...
        # Create test users
        admin = User(
            username="test_admin_duplicate",
            email="admin_duplicate@test.com",
            role="admin",
        )
        admin.set_password("password")
        db.session.add(admin)
        ...
        player = User(
            username="test_player_duplicate",
            email="player_duplicate@test.com",
            role="player",
        )
        player.set_password("password")
        db.session.add(player)
        ...
        response1 = client.post(
            "/player/request_venue_manager",
            data={"venue_id": venue.id, "notes": "First request should work"},
            follow_redirects=True,
        )
        ...
        response2 = client.post(
            "/player/request_venue_manager",
            data={"venue_id": venue.id, "notes": "Second request should fail"},
            follow_redirects=True,
        )
...
def test_admin_cannot_request_venue_management_through_ui(app, client):
    ...
    with app.app_context():
        # Create test admin
        admin = User(
            username="test_admin_excluded",
            email="admin_excluded@test.com",
            role="admin",
        )
        admin.set_password("password")
        db.session.add(admin)
        ...
        requests = VenueManagerRequest.query.filter_by(
            venue_id=test_venue.id, user_id=admin.id
        ).all()
...
def test_ui_request_includes_form_notes(app, client):
    ...
    with app.app_context():
        ...
        # Create test users
        admin = User(
            username="test_admin_notes", email="admin_notes@test.com", role="admin"
        )
        admin.set_password("password")
        db.session.add(admin)

        player = User(
            username="test_player_notes", email="player_notes@test.com", role="player"
        )
        player.set_password("password")
        db.session.add(player)
        ...
        # Create test venue
        test_venue = LocationService.create_billiard_hall(
            name="UI Notes Venue", address="UI Notes Street", city="UI Notes City"
        )
        ...
        test_notes = (
            "I have 10 years of experience managing billiard halls and tournaments"
        )

        # Make request with detailed notes
        client.post(
            "/player/request_venue_manager",
            data={"venue_id": test_venue.id, "notes": test_notes},
            follow_redirects=True,
        )
        ...
        request = VenueManagerRequest.query.filter_by(
            venue_id=test_venue.id, user_id=player.id
        ).first()
