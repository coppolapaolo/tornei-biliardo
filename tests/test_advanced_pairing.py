...
from models.matchmaking.strategies.advanced_amalfi import AdvancedAmalfiStrategy, AdvancedPairingOptions
...
class TestAdvancedAmalfiStrategy(unittest.TestCase):
    ...
    def test_generate_pairings_without_x_replacement(self):
        ...
        pairings = self.strategy._generate_pairings(processed_data, 1, preview_mode=True)
    ...
    def test_generate_x_replacement_with_challenge(self):
        ...
        with patch.object(self.strategy, '_find_suitable_challenge') as mock_find_challenge:
            mock_find_challenge.return_value = Mock()
            mock_find_challenge.return_value.name = "Test Challenge"
            ...
            self.assertIn("Challenge:", replacement.notes or "")
    ...
    def test_generate_x_replacement_with_individual_match(self):
        ...
        with patch.object(self.strategy, '_find_suitable_opponent') as mock_find_opponent:
            ...
            self.assertIn("Individual match", replacement.notes or "")
    ...
    def test_enhance_trio_match(self):
        ...
        self.assertIn("Trio match", enhanced.notes or "")
    ...
    def test_find_suitable_opponent(self):
        ...
        self.mock_prova.inscriptions = [mock_inscription1, mock_inscription2, mock_inscription3]
        ...
        with patch.object(self.strategy, '_get_active_players') as mock_get_active:
            ...
            with patch.object(self.strategy, '_find_similar_ranked_player') as mock_find_similar:
    ...
    def test_calculate_enhanced_pairing_quality_with_rematch(self):
        ...
        with patch('models.classification.models.PlayerEncounter.have_played') as mock_have_played:
            mock_have_played.return_value = True
            
            quality = self.strategy._calculate_enhanced_pairing_quality(pairing, self.mock_prova, 1)
    ...
    def test_postprocess_pairings(self):
        ...
        pairings = [Pairing(players=(1, 2), is_bye=False, round_number=1, pairing_quality=0.5)]
        
        with patch.object(self.strategy, '_calculate_enhanced_pairing_quality') as mock_calc_quality:
            mock_calc_quality.return_value = 0.8
            
            enhanced_pairings = self.strategy._postprocess_pairings(pairings, self.mock_prova, 1)
    ...
    def test_strategy_metadata(self):
        ...
        self.assertEqual(self.strategy.name, "advanced_amalfi")
        self.assertEqual(self.strategy.display_name, "Advanced Amalfi")
        self.assertEqual(self.strategy.description, "Amalfi algorithm with X-replacement, trio matches, and enhanced anti-rematch")
