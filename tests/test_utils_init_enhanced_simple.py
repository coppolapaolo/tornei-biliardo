...
class TestCreateRoundMatchesDetailed:
    ...
    def test_create_round_matches_with_inscriptions(self, mock_inscription_class, mock_match_class, mock_db):
    ...
    @patch('utils.db')
    @patch('utils.Match')
    def test_create_round_matches_odd_players_distance_mode(self, mock_match_class, mock_db):
        """Test create_round_matches with odd players when prova is not best_of."""
        # Setup prova
        mock_prova = Mock()
        mock_prova.id = 1
        mock_prova.best_of = False  # Distance mode
        mock_prova.distance = 7
...
class TestCalculateRoundClassificationDetailed:
    ...
    @patch('utils.Inscription')
    @patch('utils.Match')
    def test_calculate_round_classification_bye_match(self, mock_match_class, mock_inscription_class):
    ...
    @patch('utils.Inscription')
    @patch('utils.Match')
    def test_calculate_round_classification_completed_match(self, mock_match_class, mock_inscription_class):
        ...
        mock_inscription_class.query.filter_by.return_value.first.side_effect = [
            mock_inscription1, mock_inscription2
        ]
    ...
    @patch('utils.Inscription')
    @patch('utils.Match')
    def test_calculate_round_classification_incomplete_match(self, mock_match_class, mock_inscription_class):
    ...
    @patch('utils.Inscription')
    @patch('utils.Match')
    def test_calculate_round_classification_missing_inscription(self, mock_match_class, mock_inscription_class):
...
class TestCreateDefaultUsersDetailed:
    ...
    def test_create_default_users_complete(self, mock_user_class, mock_db):
        ...
        expected_calls = [
            (('admin', 'admin@tournament.com', 'admin'), {}),
            (('mario', 'mario@test.com', 'player'), {}),
            (('pino', 'pino@test.com', 'player'), {})
        ]
        
        actual_calls = [(call.args, call.kwargs) for call in mock_user_class.call_args_list]
        ...
        mock_db.session.add_all.assert_called_once_with([mock_admin, mock_mario, mock_pino])
        mock_db.session.commit.assert_called_once()


class TestCreateSampleTournamentDetailed:
    """Detailed tests for create_sample_tournament function."""

    @patch('builtins.print')  # Suppress print output
    @patch('utils.db')
    @patch('utils.ProvaService')
    @patch('utils.Tournament')
    def test_create_sample_tournament_complete(self, mock_tournament_class, mock_prova_service, mock_db, mock_print):
        """Test create_sample_tournament creates tournaments and proves correctly."""
        ...
        mock_tournament_class.side_effect = [mock_tournament1, mock_tournament2]
        ...
        assert mock_tournament_class.call_count == 2
        
        # Verify database operations for tournaments
        assert mock_db.session.add.call_count == 2
        assert mock_db.session.commit.call_count == 3  # 2 for tournaments + 1 final
        
        # Verify ProvaService.create_prova was called 3 times
...
class TestCreateAdminIfNotExistsDetailed:
    ...
    def test_create_admin_if_not_exists_existing_admin(self, mock_app, mock_user_class, mock_role):
        ...
        mock_user_class.query.filter_by.return_value.filter.return_value.first.return_value = mock_existing_admin
    ...
    @patch('utils.User')
    @patch('utils.current_app')
    def test_create_admin_if_not_exists_create_new(self, mock_app, mock_user_class, mock_role, mock_db):
        ...
        # No existing admin
        mock_user_class.query.filter_by.return_value.filter.return_value.first.return_value = None
    ...
    @patch('utils.User')
    @patch('utils.current_app')
    def test_create_admin_if_not_exists_no_config_dev_mode(self, mock_app, mock_user_class, mock_role):
...
class TestCreateRoundMatchesAmalfiCompatibleDetailed:
    ...
    @patch('utils.db')
    @patch('utils.Match')
    def test_create_round_matches_amalfi_compatible_even_players(self, mock_match_class, mock_db):
    ...
    @patch('utils.db')
    @patch('utils.Match')
    def test_create_round_matches_amalfi_compatible_odd_players(self, mock_match_class, mock_db):
