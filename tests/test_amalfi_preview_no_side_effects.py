...
def test_preview_round1_no_side_effects(app):
    ...
    with app.app_context():
        t = Tournament(
            name="T", tournament_type="Amalfi", is_active=True, without_x=True
        )
        db.session.add(t)
        db.session.commit()
        ...
        m0 = Match.query.count()
        e0 = PlayerEncounter.query.count()

        # Istanzio la Strategy passando le dipendenze come da design Sprint 1
        strategy = AmalfiStrategy(
            validate_fn=validate_prova, propose_fn=propose_pairings
        )
