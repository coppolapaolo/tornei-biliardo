...
class TestBackwardCompatibility:
    ...
    def test_inscription_works_both_types(self, app):
        ...
        with app.app_context():
            ...
            inscription1 = Inscription(
                user_id=player.id,
                prova_id=prova_tournament.id
            )
            inscription2 = Inscription(
                user_id=player.id,
                prova_id=prova_standalone.id
            )
    ...
    def test_prova_queries_still_work(self, app):
        ...
        with app.app_context():
            ...
            tournament_provas = Prova.query.filter_by(
                tournament_id=tournament.id
            ).all()
            assert len(tournament_provas) == 3
            
            # Filter by status
            inscription_provas = Prova.query.filter_by(
                status="inscription"
            ).all()
            ...
            prova_with_tournament = Prova.query.options(
                joinedload(Prova.tournament)
            ).filter(
                Prova.tournament_id.isnot(None)
            ).first()
    ...
    def test_null_handling_in_methods(self, app):
        ...
        with app.app_context():
            ...
            assert prova.get_winning_score() == 9  # best_of default is False, so returns distance
