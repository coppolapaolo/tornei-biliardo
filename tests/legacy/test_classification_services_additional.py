...
class TestClassificationServiceEdgeCases:
    ...
    def sample_tournament(self, db_session):
        ...
        tournament = Tournament(
            name="Classification Test Tournament",
    ...
    def test_get_player_statistics_summary_comprehensive(self, sample_prova, sample_players, db_session):
        ...
        result = ClassificationService.get_player_statistics_summary(sample_prova.tournament_id)
        
        assert result is not None
        assert isinstance(result, dict)

    def test_get_tournament_standings_simple(self, sample_prova, sample_players, db_session):
        """Test get_tournament_standings basic functionality."""
        result = ClassificationService.get_tournament_standings(sample_prova.tournament_id)
    ...
    def test_calculate_and_save_round_classification_error_handling(self, sample_prova):
        ...
        with patch('models.classification.models.RoundClassification.calculate_classification_after_round') as mock_calc:
            ...
            try:
                ClassificationService.calculate_and_save_round_classification(sample_prova.id, 1)
            except Exception:
                pass  # Expected in this test

    def test_get_player_ranking_basic(self, sample_tournament, sample_players, db_session):
        """Test get_player_ranking basic functionality."""
        player = sample_players[0]
        
        result = ClassificationService.get_player_ranking(sample_tournament.id, player.id)
...
class TestClassificationServiceHelpers:
    ...
    def test_calculate_standings_with_no_rounds(self, sample_prova):
        ...
        try:
            ClassificationService.calculate_and_save_round_classification(sample_prova.id, 0)
        except Exception:
