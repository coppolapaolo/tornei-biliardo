...
Requirements: Test cross-domain orchestration, caching, transaction management,
and extended domain functionality
"""
...
class TestCachingIntegration:
    ...
    def test_cache_basic_functionality(self, db_session):
        ...
        cache_manager.set(
            key="test_key", value={"data": "test_value"}, ttl_seconds=300, tags=["test"]
        )
...
class TestTransactionManagement:
    ...
    def test_transaction_rollback(self, db_session):
        ...
        def failing_transaction():
            user = User(
                username="rollback_test", email="rollback@test.com", role="player"
            )
            user.set_password("password")
            db_session.add(user)
    ...
    def test_nested_transactions(self, db_session):
        ...
        User.query.filter_by(username="outer").first()
        User.query.filter_by(username="inner").first()
...
class TestQueryOptimization:
    ...
    def test_query_monitoring(self, app, sample_users):
        ...
        with app.app_context():
            ...
            User.query.all()
            User.query.filter_by(username="player1").first()
...
class TestCrossDomainOrchestration:
    ...
    def test_user_onboarding_orchestration(self, app):
        ...
        with app.app_context():
            ...
            category = PlayerCategory.query.filter_by(
...
class TestExtendedDomainIntegration:
    ...
    def test_challenge_domain_integration(
        ...
        completed_attempt = ChallengeService.complete_x_replacement_attempt(
            attempt_id=attempt.id, score=75
        )
    ...
    def test_rating_system_integration(self, db_session, sample_users):
        ...
        CategoryService.assign_category(
            user_id=player2.id, category=CategoryLevel.D, assigned_by_id=director.id
        )
        ...
        # Handicap calculation may not be available, so check for either
        # handicap or explanation
...
class TestServiceLayerBoundaries:
    ...
    def test_cross_domain_data_consistency(self, app, sample_users, sample_campionato):
        ...
        with app.app_context():
            ...
            rating = PlayerRating(
                user_id=player.id, rating_system=RatingSystem.INTERNAL, rating_value=75
            )
...
class TestPerformanceIntegration:
    ...
    def test_bulk_operations_optimization(self, app, sample_users):
        ...
        with app.app_context():
            ...
            for i in range(5):
                user = User(
                    username=f"bulk_user_{i}", email=f"bulk{i}@test.com", role="player"
                )
                user.set_password("password")
                users.append(user)
