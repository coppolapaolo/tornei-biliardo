...
class TestTransactionManagement:
    ...
    def test_user_registration_with_transaction_fix(self, db_session):
        ...
        db_user = (
            db_session.query(User).filter_by(username="testuser_transaction").first()
        )
    ...
    def test_user_authentication_after_registration(self, db_session):
        ...
        authenticated_user = UserService.authenticate_user(
            "auth_test_user", "authpassword123"
        )
    ...
    def test_nested_transaction_handling(self, db_session):
        ...
        # Verify user exists in database
        db_user = (
            db_session.query(User).filter_by(username="nested_transaction_user").first()
        )
        assert db_user is not None
        assert db_user.id == user.id
