...
from models.status_enum import MatchStatus, ProvaStatus, DirectorRequestStatus
...
class TestMatchProposalRoutes:
    ...
    def test_match_proposals_route(self, mock_user, mock_service, mock_render):
        ...
        mock_render.assert_called_once_with("player/match_proposals.html", proposals=mock_proposals)
    ...
    def test_create_match_proposal_get(self, mock_request, mock_user, mock_user_class, mock_hall, mock_render):
    ...
    @patch('routes.player.current_user')
    @patch('routes.player.request')
    def test_create_match_proposal_post_direct_success(self, mock_request, mock_user, mock_service, mock_flash, mock_url_for, mock_redirect):
        ...
        mock_flash.assert_called_once_with("Match proposal created successfully! ID: 123")
    ...
    @patch('routes.player.current_user')
    @patch('routes.player.request')
    def test_create_match_proposal_post_open_success(self, mock_request, mock_user, mock_service, mock_flash, mock_url_for, mock_redirect):
        ...
        mock_flash.assert_called_once_with("Match proposal created successfully! ID: 456")
    ...
    @patch('routes.player.current_user')
    @patch('routes.player.request')
    def test_create_match_proposal_post_missing_location(self, mock_request, mock_user, mock_flash, mock_user_class, mock_hall, mock_render):
    ...
    @patch('routes.player.IndividualMatchService')
    @patch('routes.player.current_user')
    def test_accept_match_proposal_success(self, mock_user, mock_service, mock_flash, mock_url_for, mock_redirect):
    ...
    @patch('routes.player.IndividualMatchService')
    @patch('routes.player.current_user')
    def test_reject_match_proposal_success(self, mock_user, mock_service, mock_flash, mock_url_for, mock_redirect):
    ...
    @patch('routes.player.current_user')
    def test_cancel_match_proposal_success(self, mock_user, mock_service, mock_flash, mock_url_for, mock_redirect):
...
class TestPlayerDashboardAndProvaRoutes:
    ...
    @patch('routes.player.db')
    @patch('routes.player.current_user')
    def test_prova_detail_not_inscribed(self, mock_user, mock_db, mock_inscription, mock_flash, mock_url_for, mock_redirect):
    ...
    @patch('routes.player.db')
    @patch('routes.player.current_user')
    def test_prova_detail_success(self, mock_user, mock_db, mock_inscription, mock_match, mock_render):
        ...
        mock_inscription.query.filter_by.return_value.first.return_value = mock_inscription_obj
        
        mock_matches = [Mock(), Mock()]
        mock_match.query.filter_by.return_value.filter.return_value.order_by.return_value.all.return_value = mock_matches
    ...
    @patch('routes.player.current_user')
    def test_inscribe_to_prova_success(self, mock_user, mock_prova_class, mock_datetime, mock_inscription_class, mock_db, mock_flash, mock_url_for, mock_redirect):
...
class TestMatchAndRackRoutes:
    ...
    def test_match_detail_success(self, mock_db, mock_rack, mock_render):
        ...
        mock_rack.query.filter_by.return_value.order_by.return_value.all.return_value = mock_racks
        ...
        mock_render.assert_called_once_with("player/match_detail.html", match=mock_match, racks=mock_racks)
    ...
    @patch('routes.player.db')
    @patch('routes.player.current_user')
    def test_report_rack_result_success(self, mock_user, mock_db, mock_request, mock_service, mock_jsonify):
    ...
    @patch('routes.player.db')
    @patch('routes.player.current_user')
    def test_report_rack_result_invalid_winner(self, mock_user, mock_db, mock_request, mock_url_for, mock_redirect, mock_flash, mock_jsonify):
...
class TestPlayerProfileAndAccountRoutes:
    ...
    @patch('routes.player.current_user')
    @patch('routes.player.db')
    def test_profile_success(self, mock_db, mock_user, mock_inscription, mock_match, mock_classification, mock_render):
        ...
        mock_inscription.query.filter_by.return_value.join.return_value.join.return_value.order_by.return_value.all.return_value = mock_inscriptions
        ...
        mock_matches = [mock_completed_match1, mock_completed_match2, mock_pending_match]
        mock_match.query.filter.return_value.join.return_value.join.return_value.order_by.return_value.all.return_value = mock_matches
        
        # Mock classifications
        mock_classifications = [Mock(), Mock()]
        mock_classification.query.filter_by.return_value.join.return_value.order_by.return_value.all.return_value = mock_classifications
    ...
    @patch('routes.player.current_user')
    def test_request_director_success(self, mock_user, mock_director_request, mock_db, mock_flash, mock_url_for, mock_redirect):
        ...
        mock_db.session.add.assert_called_once()
        mock_db.session.commit.assert_called_once()
        mock_flash.assert_called_once_with("Richiesta inviata. Sarai contattato dall'amministratore.")
