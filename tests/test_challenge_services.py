...
class TestChallengeService:
    ...
    def test_get_user_challenges(self):
        ...
        with patch("models.challenge.services.db") as mock_db:
            with patch(
                "models.challenge.services.ChallengeService.get_active_challenges"
            ) as mock_get_active:
                ...
                mock_favorite_query.filter_by.return_value = (
                    mock_favorite_filtered_query
                )
                mock_favorite_filtered_query.all.return_value = [
                    mock_favorite1,
                    mock_favorite2,
                ]
                ...
                mock_attempt_filtered_query.all.return_value = [
                    mock_attempt1,
                    mock_attempt2,
                ]
    ...
    def test_start_challenge_attempt(self):
        ...
        with patch("models.challenge.services.db") as mock_db:
            ...
            with patch(
                "models.challenge.services.ChallengeAttempt"
    ...
    def test_complete_challenge_attempt_success(self):
        ...
        with patch("models.challenge.services.db") as mock_db:
            ...
            result = ChallengeService.complete_challenge_attempt(
                attempt_id=1, score=85, passed=True, notes="Good performance"
            )
    ...
    def test_toggle_favorite_add(self):
        ...
        with patch("models.challenge.services.db") as mock_db:
            ...
            mock_favorite = Mock()
            with patch(
                "models.challenge.services.ChallengeFavorite"
            ) as mock_favorite_class:
                ...
                mock_favorite_class.assert_called_once_with(
                    user_id=user_id, challenge_id=challenge_id
                )
    ...
    def test_get_challenge_for_x_replacement_success(self):
        ...
        with patch("models.challenge.services.db") as mock_db:
            ...
            def filter_by_side_effect(**kwargs):
                ...
                counts = {1: 3, 2: 1, 3: 2}
                mock_attempt_filtered_query.count.return_value = counts.get(
                    challenge_id, 0
                )
    ...
    def test_create_x_replacement_attempt_with_challenge_id(self):
        ...
        with patch(
            "models.challenge.services.ChallengeService.start_challenge_attempt"
    ...
    def test_create_x_replacement_attempt_without_challenge_id_success(self):
        ...
        with patch(
            "models.challenge.services.ChallengeService.get_challenge_for_x_replacement"
    ...
    def test_complete_x_replacement_attempt_success(self):
        ...
        with patch("models.challenge.services.db") as mock_db:
            ...
            with patch(
                "models.challenge.services.ChallengeService._create_x_replacement_match_result"
            ) as mock_create_result:
                # Call the method
                result = ChallengeService.complete_x_replacement_attempt(
                    attempt_id=1, score=85, notes="Excellent performance"
                )

                # Verify
                mock_db.session.get.assert_called_once_with(ChallengeAttempt, 1)
                mock_attempt.complete_attempt.assert_called_once_with(
                    score=85, passed=None
                )
    ...
    def test_complete_x_replacement_attempt_not_found(self):
        ...
        with patch("models.challenge.services.db") as mock_db:
            ...
            with patch("flask.abort") as mock_abort:
                # Call the method - should raise an exception due to abort
                with pytest.raises(Exception):
                    ChallengeService.complete_x_replacement_attempt(
                        attempt_id=999, score=85
                    )
