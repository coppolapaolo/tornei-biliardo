...
# ---------------------------------------------------------------------
#  (il resto di models.py – tutti i modelli NON utente – rimane invariato)
# ---------------------------------------------------------------------
...
class Prova(db.Model):
    ...
    def get_status_badge_info(self):
        ...
        status_map = {
            ...
            "inscription_not_started": {
                "class": "bg-info",
                "text": "Iscrizioni Future",
            },
        ...
        return status_map.get(
            real_status, {"class": "bg-secondary", "text": "Sconosciuto"}
        )
    ...
    def is_user_inscribed(self, user_id) -> bool:
        ...
        return (
            Inscription.query.filter_by(user_id=user_id, prova_id=self.id).count() > 0
        )
...
class Rack(db.Model):
    ...
    def can_be_removed(self, current_user_id):
        ...
        return (
            self.reported_by_id == current_user_id
            and not self.confirmed_by_player
            and not self.validated_by_admin
        )
...
class PlayerEncounter(db.Model):
    ...
    def __repr__(self):
        return (
            f"<PlayerEncounter {self.player1.username} "
            f"vs {self.player2.username} R{self.round_number}>"
        )
...
class RoundClassification(db.Model):
        ...
        if round_number > 1:
            ...
            for prev_class in prev_classifications:
                if prev_class.user_id in players_stats:
                    players_stats[prev_class.user_id][
                        "previous_position"
                    ] = prev_class.position

        # Ordina secondo criteri Amalfi:
        # vittorie DESC, rack_diff DESC, posizione_precedente ASC
        sorted_players = sorted(
            players_stats.items(),
            key=lambda x: (
                -x[1]["matches_won"],  # Più vittorie prima
                -x[1]["rack_difference"],  # Miglior differenza rack prima
                x[1]["previous_position"]
                or x[1]["initial_order"],  # Posizione precedente migliore prima
            ),
        )
    ...
    def __repr__(self):
        return (
            f"<RoundClassification R{self.round_number} "
            f"{self.position}° {self.user.username}>"
        )
...
class TrioMatch(db.Model):
    ...
    def add_rack_win(self, winner_id):
        ...
        if (
            self.player1_racks >= winning_score
            or self.player2_racks >= winning_score
            or self.player3_racks >= winning_score
        ):
    ...
    def __repr__(self):
        return (
            f"<TrioMatch {self.player1.username}"
            f"/{self.player2.username}/{self.player3.username}>"
        )
