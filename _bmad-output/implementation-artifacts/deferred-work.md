...
### ~~Scoperte da review spec-cancel-round-ignores-bye~~ ✅ PROCESSED (2026-04-19)

- ~~**TrioMatch orfano dopo cancel_round**~~ ✅ DONE (2026-04-19, patch in commit successivo al diff principale): aggiunto `cascade="all, delete-orphan"` alla backref `Match.trio_match` (`models/match/models.py`). Ora tutti i `db.session.delete(match)` (4 call-site: `round_manager.cancel_round`, `round_cancellation.py:89`, `round_cancellation.py:176`, `services.py:710`) cascano automaticamente sul `TrioMatch` associato. Regression test `TestCancelRoundCascadesTrioMatch::test_cancel_round_with_trio_pending_removes_trio_match` in `tests/new/unit/test_cancel_round_bye.py`.

- ~~**MatchResult orfano dopo cancel_round**~~ REJECTED (2026-04-19): verifica empirica — `grep "MatchResult("` su tutto il codebase ritorna zero constructor calls in production (solo la definizione della classe e un `overlaps="match_results"` su `User.match_results`). `MatchResult` è una classe dormant/legacy: se nessuno la istanzia, nessun record esiste in DB, quindi nessun orfano possibile. Bug teorico non realizzabile. Se in futuro qualcuno aggiungesse `MatchResult(...)` ai servizi, andrà rivalutato.
