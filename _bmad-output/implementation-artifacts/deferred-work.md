...
- **G3** ✅ RESOLVED as misdiagnosis (2026-04-19): `ADR-026` — l'intent del reset è "correzione score, pair preservato". Il vero difetto era nel lato opposto (cleanup di `PlayerEncounter` dentro il reset + precondition errata in `cancel_round`). Implementazione in commit che include: rimozione `delete_encounter`, precondition fix `(score or 0) > 0` in `cancel_round`, test invertito `test_contract_reset_preserves_encounter`, glossario in `models/competition/CLAUDE.md`. ADR-002 annotato "superseded in part".

---

### Residui da ADR-026 (Edge Case Hunter findings, 2026-04-19)

Scoperti durante review adversariale di ADR-026 ma **pre-existing al diff**, tracciati per futura attenzione.

- ~~**All-bye round è un-cancellabile**~~ ✅ DONE (2026-04-19): Spec `spec-cancel-round-ignores-bye.md`.

### Scoperte da review spec-cancel-round-ignores-bye (2026-04-19)

Pre-existing, scope-adiacenti ma non introdotte dal diff. Tracciate per futura attenzione.

- **TrioMatch orfano dopo cancel_round** (MED, Edge Case Hunter): `TrioMatch.match_id` è FK verso `Match.id` senza `ON DELETE CASCADE` a livello DB (SQLite con `PRAGMA foreign_keys=0`) e senza `cascade="all, delete-orphan"` sulla relationship ORM (`models/match/models.py:471`). `AdvancedRoundManager.cancel_round` chiama `db.session.delete(match)` ma non cancella il `TrioMatch` associato → orfano con `match_id` danglante. Scenario raggiungibile: gara con numero dispari di iscritti → trio pending in un round → director cancella il round. Valutare se aggiungere cascade ORM-side o cleanup esplicito in `cancel_round`.

- **MatchResult orfano dopo cancel_round** (LOW, Edge Case Hunter): `MatchResult.match_id` (`models/match/models.py:407`) è FK senza `ondelete="CASCADE"` DB-side e senza relationship cascade Match→MatchResult ORM-side. Dopo `cancel_round`, `MatchResult` resta con `match_id` orfano. Scenario: match PENDING con report di risultato proposto (MatchResult creato) ma non ancora committed in `match.scores` → non entra nel filtro `matches_with_results` → cancellato → `MatchResult` orfano. Valutare se aggiungere cascade.
 Fix minimo in `AdvancedRoundManager.cancel_round`: il predicato `matches_with_results` ora esclude i match con `is_bye=True`. Il bye è artefatto algoritmico (parity dispari), non input utente: `player1_score = round_distance` è solo convenzione di persistenza per la classification → non deve contare come "risultato parziale". I walkover (2-player forfeit, trio walkover) restano inclusi: rappresentano azioni umane (forfeit) e vanno resettati deliberatamente prima di cancellare il round. `reset_match_complete` su bye continua a raisare `ValueError("Non puoi resettare una partita bye!")` (status quo, coerente con "bye sempre in stato iniziale"). 6 unit test in `test_cancel_round_bye.py` (succeed con solo bye + pending, succeed all-bye, blocked con real score, blocked con walkover 2-player, blocked con trio walkover, reset bye still blocked).

- ~~**Tiebreaker rows sopravvivono al reset**~~ ✅ DONE (2026-04-19, commit `10d6ff6`): riqualificato in interview 2026-04-19 come "bloccare il reset quando la gara ha uno SSR attivo". Implementato via nuova precondition in `AdvancedRoundManager.can_modify_match` (check `Tiebreaker.status != CANCELLED` su `gara_id`). UI nasconde automaticamente il bottone reset tramite il dict `match_can_modify` già consumato da `_match_card.html` e `_match_result_row.html`; `_match_admin_controls.html` aggiornato con flag esplicito passato da `routes/admin/match/detail.py`. 7 unit + 3 integration test. Spec: `spec-reset-blocked-by-tiebreaker.md` (done).

### ~~Residui da review spec-reset-blocked-by-tiebreaker~~ ✅ ALL DONE (2026-04-19)

Scoperti dai 3 reviewer (Blind Hunter + Edge Case Hunter + Acceptance Auditor) durante step-04. Tutti chiusi in una singola sessione di cleanup.

- ~~**Tiebreaker campionato-level (`gara_id=NULL`) non intercettato**~~ ✅ DONE (2026-04-19): `AdvancedRoundManager._gara_has_active_tiebreaker` ora applica filtro OR su `Tiebreaker.gara_id == gara_id` **OR** `Tiebreaker.campionato_id == gara.campionato_id` (se `gara.campionato_id IS NOT NULL`). Un TB playoff campionato sul match di gara X (con X sotto campionato Y) blocca il reset. Gare standalone non matchano TB campionato-level. 2 regression test in `TestResetBlockedByCampionatoTiebreaker`.

- ~~**`cancel_round` non bloccato da TB attivo**~~ ✅ DONE (2026-04-19): `cancel_round` ora chiama `_gara_has_active_tiebreaker` come precondizione (prima del check `round_number < gara.current_round`). Stringa i18n dedicata ("...per cancellare il turno"). Il director deve annullare lo spareggio per cancellare qualsiasi round, coerente con il blocco del reset single-match. 2 regression test in `TestCancelRoundBlockedByTiebreaker`.

- ~~**Traduzioni EN corrotte (pre-existing)**~~ ✅ DONE (2026-04-19): 3 `msgstr` in `translations/en/LC_MESSAGES/messages.po` avevano due traduzioni concatenate senza separatore. Ripuliti manualmente (tenuta la versione meglio formulata): `models/kpi/notifications.py:56` (platform match count), `routes/player/exports.py:514` (export notification), `templates/individual_match/match_detail.html:352` (forfeit confirmation). Recompile OK (1389 translated messages, 0 untranslated).
