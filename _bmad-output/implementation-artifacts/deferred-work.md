...
- **Tiebreaker rows sopravvivono al reset**: `Match.tiebreakers` è `cascade="all, delete-orphan"` solo in caso di delete del Match (non del reset). Dopo reset, la match row persiste, quindi eventuali `Tiebreaker` in stato `COMPLETED` con `winner_id` settato restano allegati al match azzerato. Se il match viene ri-completato dopo reset, il classification aggregator può leggere dati tiebreaker stale. Comportamento pre-esistente, non introdotto da ADR-026. Decidere: (a) `reset_match_complete` cancella anche tiebreaker del match, (b) `MatchStateService.to_completed` rigenera tiebreaker se stale, (c) accettare come limitazione documentata in ADR-026 Operations Glossary.
