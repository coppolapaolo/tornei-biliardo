...
- **All-bye round è un-cancellabile**: `AdvancedRoundManager.cancel_round` usa `(score or 0) > 0` come precondition "matches with results". I match bye vengono creati con `player1_score = round_distance` (es. 5) in `round_creation.py:36-43` → cancel blocca. E `reset_match_complete` rifiuta match bye (`ValueError("Non puoi resettare una partita bye!")`). Risultato: un round che contiene almeno un bye non può essere cancellato senza manipolare i dati direttamente. Scenario limite ma documentato. Comportamento pre-esistente, non introdotto da ADR-026. Valutare se estendere `reset_match_complete` per accettare bye (azzerando `player1_score` a 0) oppure se scrivere una "cancel_round force" che ignora la precondition per match bye.
...
### Residui da review spec-reset-blocked-by-tiebreaker (2026-04-19)

Scoperti dai 3 reviewer (Blind Hunter + Edge Case Hunter + Acceptance Auditor) durante step-04. Tutti DEFER (non in scope di questa feature).

- **Tiebreaker campionato-level (`gara_id=NULL`) non intercettato**: un `Tiebreaker` con `campionato_id=X` e `gara_id=NULL` (es. playoff campionato) punta via `match_id` a un match di gara, ma il nostro filtro su `Tiebreaker.gara_id == gara.id` lo ignora. Reset passa anche con playoff TB attivo sul match. Scope adiacente — richiede interview PO per decidere se la "certificazione" si estende anche ai TB campionato-level o se è esplicitamente limitata ai TB di gara. Severity HIGH se il caso è realistico; MEDIUM se il PO chiarisce che TB campionato-level punta a match dummy/dedicati. Rivedere dopo verifica su produzione.

- **`cancel_round` non bloccato da TB attivo**: il blocco vive solo in `can_modify_match` (usato da reset). `cancel_round` continua a funzionare anche con `Tiebreaker` attivo sulla gara, aprendo incoerenza semantica — un director può cancellare l'intero round corrente (con tutti i suoi match) anche se la gara è "certificata" da SSR. Il caso è raro perché cancel_round ha già una precondizione stringente (no match con risultati parziali), ma se il round è appena stato creato e nessun match ha score, la cancellazione passa. Da valutare se estendere il check TB anche a `cancel_round` o se accettare l'incoerenza con nota in ADR-026.

- **Traduzioni EN corrotte (pre-existing)**: almeno 3 `msgstr` in `translations/en/LC_MESSAGES/messages.po` hanno due traduzioni concatenate senza separatore (es. `"...is growing!%(value)s matches have been played..."`). File affetti noti da findings: `routes/player/exports.py:514`, `templates/individual_match/match_detail.html:352`, entry su KPI "platform match count". Non introdotte da questo commit. Fix facile: grep + manual cleanup sul .po, poi recompile.
