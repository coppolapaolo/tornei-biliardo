...
- **Tiebreaker rows sopravvivono al reset** → **riqualificato in interview 2026-04-19**: il framing corretto non è "cancellare/rigenerare tiebreaker" ma "**bloccare il reset quando la gara ha uno SSR attivo**". Il PO ha chiarito che lo SSR certifica implicitamente l'integrità dei risultati — non si torna indietro. Decisione: nuova precondition in `AdvancedRoundManager.can_modify_match` + hide bottone reset nella UI quando la gara ha `Tiebreaker` con stato != `CANCELLED`. Scope: tutti i match della gara (Q3:B), nessun override admin (Q3b:i), tutti gli stati non-cancelled bloccano (Q4:C). Spec: `spec-reset-blocked-by-tiebreaker.md` (status: draft, pronto per quick-dev).
