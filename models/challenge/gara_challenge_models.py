...
class GaraChallenge(BaseModel, TimestampMixin):
    ...
    max_attempts = db.Column(db.Integer, nullable=False, default=1)  # Attempts per player
    ...
    __table_args__ = (
        db.UniqueConstraint("gara_id", "challenge_id", "round_number", name="uq_gara_challenge_round"),
    )
...
class GaraChallengeAttempt(BaseModel, TimestampMixin):
    ...
    gara_challenge_id = db.Column(
        db.Integer, db.ForeignKey("gara_challenge.id", ondelete="CASCADE"), nullable=False
    )
    user_id = db.Column(
    ...
    __table_args__ = (
        db.UniqueConstraint("gara_challenge_id", "user_id", "attempt_number", name="uq_gara_challenge_user_attempt"),
    )
    ...
    def complete_attempt(
        ...
        if self.gara_challenge.challenge.pass_fail_only:
            self.passed = passed
            ...
            if passed is None and score is not None:
                passing_score = self.gara_challenge.challenge.max_score * 0.7
...
class GaraChallengeClassification(BaseModel):
    ...
    total_best_score = db.Column(db.Integer, nullable=False, default=0)  # Sum of best attempts across challenges
    total_all_attempts = db.Column(db.Integer, nullable=False, default=0)  # Sum of all attempts for tiebreaking
    challenges_completed = db.Column(db.Integer, nullable=False, default=0)  # Number of challenges attempted
    ...
    __table_args__ = (
        db.UniqueConstraint("gara_id", "user_id", name="uq_gara_challenge_classification"),
    )
    ...
    def calculate_for_gara(cls, gara_id: int) -> List["GaraChallengeClassification"]:
        ...
        gara_challenges = GaraChallenge.query.filter_by(gara_id=gara_id, is_active=True).all()
        ...
        for user in inscribed_users:
            ...
            for gara_challenge in gara_challenges:
                ...
                completed_attempts = [attempt for attempt in user_attempts if attempt.completed]
                
                if completed_attempts:
                    challenges_completed += 1
                    # Get best score for this challenge
                    best_score = max(attempt.score or 0 for attempt in completed_attempts)
                    total_best_score += best_score
                    # Sum all attempts for this challenge
                    total_all_attempts += sum(attempt.score or 0 for attempt in completed_attempts)

            # Update or create classification record
            classification = cls.query.filter_by(gara_id=gara_id, user_id=user.id).first()
            if not classification:
                classification = cls(gara_id=gara_id, user_id=user.id)
        ...
        classifications.sort(key=lambda x: (-x.total_best_score, x.total_all_attempts))
    ...
    def get_user_challenge_details(self) -> List[Dict[str, Any]]:
        ...
        gara_challenges = GaraChallenge.query.filter_by(gara_id=self.gara_id, is_active=True).all()
        details = []
        ...
        for gara_challenge in gara_challenges:
            ...
            best_score = max((attempt.score or 0 for attempt in completed_attempts), default=0)
