...
class Challenge(BaseModel, TimestampMixin):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        unique_players = attempts_query.with_entities(ChallengeAttempt.user_id).distinct().count()
        ...
        if self.pass_fail_only:
            pass_rate = sum(1 for attempt in attempts_query if attempt.passed) / total_attempts * 100
        else:
            # Consider passing as achieving 70% of max score
            passing_score = self.max_score * 0.7
            pass_rate = sum(1 for score in scores if score >= passing_score) / total_attempts * 100
...
class ChallengeAttempt(BaseModel, TimestampMixin):
    ...
    prova_id = db.Column(db.Integer, db.ForeignKey("prova.id", ondelete="SET NULL"), nullable=True)
    ...
    def complete_attempt(self, score: Optional[int] = None, passed: Optional[bool] = None) -> None:
    ...
    def __repr__(self) -> str:
        return f"<ChallengeAttempt {self.user_id} -> {self.challenge.name}: {self.score}>"
