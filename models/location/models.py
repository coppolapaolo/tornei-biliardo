...
class UserLocationAvailability(BaseModel, TimestampMixin):
    ...
    billiard_hall_id = db.Column(db.Integer, db.ForeignKey("billiard_hall.id", ondelete="CASCADE"), nullable=False)
    
    # Availability settings
    is_available = db.Column(db.Boolean, nullable=False, default=True)
    available_days = db.Column(db.String(20), nullable=True)  # JSON array of day numbers
    ...
    advance_notice_hours = db.Column(db.Integer, nullable=False, default=24)  # Minimum advance notice
    ...
    billiard_hall = db.relationship("BilliardHall", back_populates="user_availabilities")
    
    # Unique constraint: one availability record per user per location
    __table_args__ = (
        db.UniqueConstraint('user_id', 'billiard_hall_id', name='uq_user_location_availability'),
    )
    ...
    def is_available_at(self, check_datetime: datetime) -> bool:
        ...
        if self.preferred_time_start and self.preferred_time_end:
            ...
            if self.preferred_time_start <= self.preferred_time_end:
                # Normal time range
                if not (self.preferred_time_start <= check_time <= self.preferred_time_end):
                    return False
            else:
                # Overnight time range
                if not (check_time >= self.preferred_time_start or check_time <= self.preferred_time_end):
                    return False
...
class LocationReview(BaseModel, TimestampMixin):
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'billiard_hall_id', name='uq_user_location_review'),
    )
    
    def get_overall_rating(self) -> float:
        """Calculate overall rating from individual aspects."""
        aspects = [self.table_quality, self.atmosphere, self.service, self.value_for_money]
