...
class PlayerCategory(BaseModel, TimestampMixin):
    ...
    __table_args__ = (
        db.Index('idx_user_active_category', user_id, is_active),
    )
...
class PlayerRating(BaseModel, TimestampMixin):
    ...
    @classmethod
    def get_user_rating(cls, user_id: int, rating_system: RatingSystem) -> Optional['PlayerRating']:
...
class HandicapRule(BaseModel, TimestampMixin):
    ...
    def calculate_category_handicap(self, higher_category: CategoryLevel, lower_category: CategoryLevel) -> int:
    ...
    def calculate_rating_handicap(self, higher_rating: int, lower_rating: int, rating_system: RatingSystem) -> int:
...
class CategoryHandicapRule(BaseModel):
    ...
    id = db.Column(db.Integer, primary_key=True)
    rule_id = db.Column(db.Integer, db.ForeignKey("handicap_rule.id", ondelete="CASCADE"), nullable=False)
    ...
    handicap_value = db.Column(db.Integer, nullable=False)  # Points advantage for lower category
    ...
    # Unique constraint
    __table_args__ = (
        db.UniqueConstraint('rule_id', 'higher_category', 'lower_category', name='uq_rule_categories'),
    )
...
class RatingHandicapRule(BaseModel):
    ...
    rating_system = db.Column(db.Enum(RatingSystem), nullable=False)
    min_difference = db.Column(db.Integer, nullable=False, default=50)  # Minimum rating diff for handicap
    max_difference = db.Column(db.Integer, nullable=True)  # Maximum rating diff to consider
    points_per_handicap = db.Column(db.Integer, nullable=False, default=100)  # Rating points per handicap point
