...
class Exam(BaseModel, TimestampMixin):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        unique_students = (
            attempts_query.with_entities(ExamAttempt.user_id).distinct().count()
        )
        ...
        for letter, value in sorted(
            grade_values.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        completion_rate = (
            (total_attempts / total_started * 100) if total_started > 0 else 0
        )
...
class ExamChallenge(BaseModel):
    ...
    def __repr__(self) -> str:
        return (
            f"<ExamChallenge {self.exam.name} -> {self.challenge.name} ({self.order})>"
        )
