...
class DomainOrchestrator:
    ...
    def setup_complete_tournament(
        ...
        try:
            ...
            if auto_assign_categories:
                category_assignments = self._auto_assign_player_categories(tournament.id)
            ...
            logger.info(f"Tournament setup completed: {tournament.name} ({tournament.id})")
        ...
        try:
            ...
            if user.is_player:
                ...
                if welcome_challenges:
                    onboarding_steps.append(f"created_{len(welcome_challenges)}_welcome_challenges")
            ...
            result_data = {
                ...
                "category_assignment": category_assignment.id if category_assignment else None,
    ...
    def get_orchestration_metrics(self) -> Dict[str, Any]:
        ...
        avg_execution_time = sum(op.execution_time_ms for op in self._operation_history) / total_ops
        
        # Count by operation type
        type_counts = {}
        for op_type in OperationType:
            count = len([op for op in self._operation_history if op.operation_type == op_type])
    ...
    def _auto_assign_player_categories(self, tournament_id: int) -> List[Dict[str, Any]]:
    ...
    def _setup_tournament_handicap_rules(self, tournament_id: int) -> List[Dict[str, Any]]:
