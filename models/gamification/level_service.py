...
class LevelService:
    ...
    def award_xp(
        ...
        Args:
            ...
            related_entities: Optional dict of related entities
                (e.g., {"match_id": 123})
        ...
        while True:
            ...
            # Guardia anti-loop infinito: una misconfigurazione admin
            # (level_base_xp=0 o level_power=0) può rendere xp_needed<=0, per
            # cui `current_xp >= xp_needed` è sempre vera e `current_xp -=
            # xp_needed` non decrementa mai → hang/DoS. Interrompi e logga.
            if xp_needed <= 0:
                logger.warning(
                    "get_xp_for_next_level=%s al livello %s: level-up "
                    "interrotto per evitare un loop infinito (config XP errata).",
                    xp_needed,
    ...
    def get_level_progress(user_id: int) -> Dict[str, Any]:
        ...
        progress_percentage = (
            (user_level.current_xp / xp_for_next * 100) if xp_for_next > 0 else 0.0
        )
    ...
    def get_user_level_stats(user_id: int) -> Dict[str, Any]:
        ...
        xp_by_type = {
            txn_type.value: int(total_xp) for txn_type, total_xp in xp_by_type_query
        }
