...
class AdvancedRoundManager:
    ...
    def get_round_lock_status(gara_id: int, round_number: int) -> RoundLockStatus:
        ...
        # Round is locked if a subsequent round has been started
        if round_number < gara.current_round:
            # Check if any subsequent round has matches
        ...
        # Current round is partially locked if some matches are completed
        if round_number == gara.current_round:
            ...
            completed_matches = [m for m in round_matches 
    ...
    def can_modify_match(match_id: int) -> Tuple[bool, str]:
        ...
        if lock_status == RoundLockStatus.LOCKED:
            return False, "Il turno è bloccato perché un turno successivo è già iniziato"
    ...
    @staticmethod
    def reset_match_with_validation(match_id: int, admin_override: bool = False) -> Tuple[bool, str]:
        ...
        try:
            # Reset the match
            MatchService.reset_match(match_id)
    ...
    @staticmethod
    def cancel_round(gara_id: int, round_number: int, admin_override: bool = False) -> Tuple[bool, str]:
        ...
        if not admin_override:
            matches_with_results = [m for m in round_matches 
                                  if m.player1_score is not None or m.player2_score is not None]
            if matches_with_results:
                return False, "Alcuni match hanno già risultati parziali. Usa override admin per procedere."
    ...
    @staticmethod
    def bulk_reset_round_matches(gara_id: int, round_number: int) -> Tuple[bool, str, Dict[str, int]]:
        ...
        if not completed_matches:
            return False, f"Nessun match completato trovato nel turno {round_number}", {}
    ...
    def get_round_modification_summary(gara_id: int) -> Dict[int, Dict[str, Any]]:
        ...
        for (round_number,) in rounds_with_matches:
            lock_status = AdvancedRoundManager.get_round_lock_status(gara_id, round_number)
            ...
            completed_matches = [m for m in round_matches 
                               if m.status == MatchStatus.COMPLETED.value]
            pending_matches = [m for m in round_matches 
                             if m.status == MatchStatus.PENDING.value]
            ...
            rounds_summary[round_number] = {
                ...
                "can_bulk_reset": len(completed_matches) > 0 and lock_status != RoundLockStatus.LOCKED
            }

        return rounds_summary

    @staticmethod
    def _recalculate_affected_classifications(gara_id: int, affected_round: int) -> None:
        """Recalculate classifications for affected rounds after a match modification."""
    ...
    @staticmethod
    def _update_round_progression_after_reset(gara_id: int, affected_round: int) -> None:
        ...
        if len(completed_matches) < len(round_matches) and affected_round == gara.current_round:
            # Check if subsequent rounds exist and delete them if they have no results
            subsequent_rounds = (
                db.session.query(Match.round_number)
                .filter(
                    Match.gara_id == gara_id,
                    Match.round_number > affected_round
                )
                .distinct()
                .all()
            )
            ...
            for (subsequent_round,) in subsequent_rounds:
                subsequent_matches = Match.query.filter_by(
                    gara_id=gara_id,
                    round_number=subsequent_round
                ).all()

                # If subsequent round has no completed matches, it can be safely removed
                subsequent_completed = [m for m in subsequent_matches
                                     if m.status == MatchStatus.COMPLETED.value]
        ...
        for round_num in range(1, gara.rounds_count + 1):
            ...
            if round_matches:
                completed = [m for m in round_matches 
                           if m.status == MatchStatus.COMPLETED.value]
