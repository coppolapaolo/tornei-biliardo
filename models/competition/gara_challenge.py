...
Design Note (Sprint 11 - December 2025):
    This module was moved from models/challenge/ to models/competition/ to properly
    establish domain boundaries. The Competition domain owns the integration between
    tournaments (Gara) and challenges. The Challenge domain remains a pure domain
    that doesn't know about competitions.
...
class GaraChallenge(BaseModel, TimestampMixin):
    ...
    logic (when to execute challenges, how many attempts, etc.) is
    competition-specific, not challenge-specific.
    """
