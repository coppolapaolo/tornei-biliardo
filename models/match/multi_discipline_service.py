...
class MultiDisciplineService:
    ...
    def configure_rotating_disciplines(
        ...
        if rotation_type == "set_level":
            ...
            for i, match_set in enumerate(match.sets):
    ...
    def configure_custom_disciplines(
        match_id: int,
        set_configurations: Dict[int, Dict[str, Any]]
    ) -> None:
        ...
        for set_number, config in set_configurations.items():
            match_set = next((s for s in match.sets if s.set_number == set_number), None)
            if not match_set:
                continue
    ...
    def get_available_disciplines() -> List[Dict[str, str]]:
        ...
        return [
            {"value": "palla_8", "label": "8-Ball", "description": "Standard 8-ball pool"},
            {"value": "palla_9", "label": "9-Ball", "description": "9-ball rotation"},
            {"value": "palla_10", "label": "10-Ball", "description": "10-ball rotation"},
            {"value": "straight_pool", "label": "Straight Pool", "description": "14.1 continuous"},
            {"value": "one_pocket", "label": "One Pocket", "description": "One pocket pool"},
            {"value": "bank_pool", "label": "Bank Pool", "description": "Bank shot pool"},
            {"value": "rotation", "label": "Rotation", "description": "15-ball rotation"},
        ]
    ...
    def validate_discipline_configuration(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "disciplines" in config:
            available = [d["value"] for d in MultiDisciplineService.get_available_disciplines()]
            for discipline in config["disciplines"]:
                if discipline not in available:
                    errors.append(f"Invalid discipline: {discipline}")
        
        # Check for conflicts
        if "rotation_type" in config and config["rotation_type"] not in ["set_level", "rack_level"]:
    ...
    def get_match_discipline_progress(match_id: int) -> Dict[str, Any]:
        ...
        progress_info = {
            ...
            "completed_sets": len([s for s in match.sets if s.is_completed()]),
        ...
        for match_set in match.sets:
            ...
            if current_set and match_set.set_number == current_set.set_number:
                if match_set.is_multi_discipline:
                    ...
                    set_info["next_rack_discipline"] = match_set.get_discipline_for_rack(next_rack)
                else:
