...
class Set(BaseModel, TimestampMixin):
    ...
    def configure_multi_discipline(self, disciplines: List[str], mode: str = "rotation") -> None:
        ...
        if not disciplines or len(disciplines) < 2:
            raise ValueError("At least 2 disciplines required for multi-discipline mode")
    ...
    def get_discipline_for_rack(self, rack_number: int) -> str:
        ...
        if not self.is_multi_discipline:
            return self.discipline or getattr(self.match, 'discipline', 'palla_8') or "palla_8"
        
        # Check specific assignment first
        if self.discipline_assignment and str(rack_number) in self.discipline_assignment:
    ...
    def get_discipline_summary(self) -> Dict[str, Any]:
        ...
        if not self.is_multi_discipline:
            discipline = self.discipline or getattr(self.match, 'discipline', 'palla_8') or "palla_8"
            return {
        ...
        for rack in self.racks:  # type: ignore
            ...
            discipline_counts[rack_discipline] = discipline_counts.get(rack_discipline, 0) + 1
    ...
    def add_rack_result(self, winner_id: int, rack_number: Optional[int] = None, discipline_override: Optional[str] = None) -> 'SetRack':
        ...
        rack = SetRack(
            ...
            winner_id=winner_id,
            discipline_override=rack_discipline if rack_discipline != self.get_discipline_for_rack(rack_number) else None
        )
...
class SetRack(BaseModel, TimestampMixin):
    ...
    discipline_override = db.Column(db.String(50), nullable=True)  # Different discipline for this rack
    ...
    def get_effective_discipline(self) -> str:
        ...
        return self.discipline_override or self.set.get_discipline_for_rack(self.rack_number)
