...
class QueryMetrics:
    ...
    def add_execution(self, duration_ms: float, params: Optional[Dict] = None, stack_trace: Optional[str] = None):
...
class QueryAnalyzer:
    ...
    def record_query(
        ...
        with self._lock:
            ...
            if (duration_ms > self.slow_query_threshold_ms or 
                metrics.execution_count % self.frequent_query_threshold == 0):
    ...
    def detect_n1_problems(self, time_window_minutes: int = 5) -> List[N1Problem]:
        ...
        recent_queries = {
            hash_key: metrics for hash_key, metrics in self._query_metrics.items()
        ...
        for parent_hash, parent_metrics in recent_queries.items():
            if parent_metrics.execution_count == 1:  # Potential parent query
                ...
                for child_hash, child_metrics in recent_queries.items():
                    if (child_hash != parent_hash and 
                        child_metrics.execution_count >= self.n1_detection_threshold and
                        self._queries_are_similar(parent_metrics.sql_text, child_metrics.sql_text)):
                        child_candidates.append(child_metrics.sql_text)
                
                if child_candidates:
                    severity = self._calculate_n1_severity(len(child_candidates))
                    solution = self._suggest_n1_solution(parent_metrics.sql_text, child_candidates)
        ...
        with self._lock:
            ...
            self._n1_problems = [
                p for p in self._n1_problems 
                if (datetime.utcnow() - p.detection_time).total_seconds() < 3600  # 1 hour
            ]
    ...
    def get_optimization_suggestions(self, limit: int = 10) -> List[OptimizationSuggestion]:
        ...
        with self._lock:
            ...
            slow_queries = [
                metrics for metrics in self._query_metrics.values()
                if metrics.avg_time_ms > self.slow_query_threshold_ms
            ]
            
            for metrics in sorted(slow_queries, key=lambda m: m.avg_time_ms, reverse=True)[:limit]:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_execution_time = sum(m.avg_time_ms for m in self._query_metrics.values()) / total_queries if total_queries > 0 else 0
            ...
            return {
                ...
                "slowest_queries": [
                    {
                        "sql": m.sql_text[:200] + "..." if len(m.sql_text) > 200 else m.sql_text,
                        "avg_time_ms": round(m.avg_time_ms, 2),
                        "execution_count": m.execution_count,
                        "total_time_ms": round(m.total_time_ms, 2)
                    }
                    for m in sorted(slow_queries, key=lambda x: x.avg_time_ms, reverse=True)[:10]
                ],
                "most_frequent_queries": [
                    ...
                    for m in sorted(frequent_queries, key=lambda x: x.execution_count, reverse=True)[:10]
                ],
                "n1_problems": [
                    {
                        "parent_query": p.parent_query[:200] + "..." if len(p.parent_query) > 200 else p.parent_query,
    ...
    def _analyze_query(self, metrics: QueryMetrics) -> None:
        ...
        if metrics.avg_time_ms > self.slow_query_threshold_ms * 2:
            logger.warning(f"Very slow query detected: {metrics.avg_time_ms:.2f}ms average")
    ...
    def _queries_are_similar(self, query1: str, query2: str) -> bool:
        ...
        return (len(normalized1) > 20 and 
                len(normalized2) > 20 and
                self._calculate_similarity(normalized1, normalized2) > 0.8)
    ...
    def _generate_optimization_suggestion(self, metrics: QueryMetrics) -> Optional[OptimizationSuggestion]:
...
class QueryOptimizer:
    ...
    def get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        ...
        for suggestion in suggestions:
            priority = "medium" if suggestion.suggestion_type in ["index", "caching"] else "low"
            recommendations.append({
                "priority": priority,
                "type": suggestion.suggestion_type,
                "title": suggestion.description,
                "description": suggestion.code_example or "See implementation guide",
