...
class StrategyConfiguration:
    ...
    def from_gara(cls, gara) -> 'StrategyConfiguration':
        ...
        return cls(
            ...
            rating_type=RatingType(gara.rating_type) if hasattr(gara, 'rating_type') and gara.rating_type else None
        )
...
def calculate_rounds_for_strategy(strategy: MatchmakingStrategy, num_players: int) -> int:
...
def get_trio_configuration(distance: int) -> Optional[Dict]:
    ...
    trio_configs = {
        3: {"mini_rounds": 1, "racks_per_player": 2, "total_racks": 3, "scoring": "1 + racks_won"},
        4: {"mini_rounds": 2, "racks_per_player": 4, "total_racks": 6, "scoring": "racks_won"},
        5: {"mini_rounds": 2, "racks_per_player": 4, "total_racks": 6, "scoring": "1 + racks_won"},
        6: {"mini_rounds": 3, "racks_per_player": 6, "total_racks": 9, "scoring": "racks_won"},
        7: {"mini_rounds": 3, "racks_per_player": 6, "total_racks": 9, "scoring": "1 + racks_won"}
    }
...
def validate_strategy_change(
    ...
    if old_strategy in [MatchmakingStrategy.DIRECT_ELIMINATION, MatchmakingStrategy.DOUBLE_KNOCKOUT]:
        if new_strategy not in [MatchmakingStrategy.DIRECT_ELIMINATION, MatchmakingStrategy.DOUBLE_KNOCKOUT]:
