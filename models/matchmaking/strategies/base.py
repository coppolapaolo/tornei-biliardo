...
class BaseStrategy(ABC):
    ...
    def validate(self, prova: object) -> ValidationResult:
        ...
        if not hasattr(prova, 'inscriptions'):
            ...
            if player_count < self.min_players:
                errors.append(f"Insufficient players: {player_count} < {self.min_players}")
            ...
            if player_count % 2 != 0 and not self.supports_byes:
                errors.append(f"Strategy does not support odd number of players: {player_count}")
    ...
    def preview(self, prova: object, round_number: int) -> Sequence[Pairing]:
        ...
        pairings = self._generate_pairings(processed_data, round_number, preview_mode=True)
    ...
    def propose(self, prova: object, round_number: int) -> Sequence[Pairing]:
        ...
        # Generate pairings (strategy-specific)
        pairings = self._generate_pairings(processed_data, round_number, preview_mode=False)
    ...
    def _record_metrics(
        ...
        if self._execution_start:
            ...
            avg_quality = sum(p.pairing_quality for p in pairings) / len(pairings) if pairings else 0.0
