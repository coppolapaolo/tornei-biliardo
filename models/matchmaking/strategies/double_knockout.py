...
class DoubleKnockoutStrategy(PairingStrategy):
        ...
        try:
            ...
            if player_count < 4:
                return ValidationResult(
                    ok=False,
                    messages=("Double Knockout requires at least 4 players",)
                )
            
            if player_count > 64:
                return ValidationResult(
                    ok=False,
                    messages=("Double Knockout with more than 64 players may be impractical",)
                )
    ...
    def _generate_subsequent_round_pairings(self, prova: "Prova", round_number: int) -> List[Pairing]:
        ...
        all_matches = Match.query.filter_by(
            prova_id=prova.id,
            status="completed"
        ).filter(Match.round_number < round_number).all()
        ...
        if bracket_info["phase"] == "winners_bracket":
            pairings.extend(self._generate_winners_bracket_pairings(
                prova, round_number, player_status, bracket_info
            ))
        elif bracket_info["phase"] == "losers_bracket":
            pairings.extend(self._generate_losers_bracket_pairings(
                prova, round_number, player_status, bracket_info
            ))
            ...
            pairings.extend(self._generate_grand_final_pairings(
                prova, round_number, player_status
            ))
    ...
    def _determine_bracket_phase(self, prova: "Prova", round_number: int, player_status: Dict[int, str]) -> Dict[str, Any]:
        ...
        if len(active_players) > 1 and len(eliminated_once) > 1:
            return {"phase": "mixed", "winners_active": len(active_players), "losers_active": len(eliminated_once)}
            ...
            return {"phase": "grand_final", "winner_champ": active_players[0], "loser_champ": eliminated_once[0]}
        else:
    ...
    def _generate_winners_bracket_pairings(self, prova: "Prova", round_number: int, 
        ...
        previous_winners_matches = Match.query.filter_by(
            prova_id=prova.id,
            round_number=round_number - 1,
            status="completed"
        ).filter(Match.notes != "losers_bracket").all()  # Assuming we mark losers bracket matches
    ...
    def _generate_losers_bracket_pairings(self, prova: "Prova", round_number: int, 
        ...
        previous_losers_survivors = self._get_previous_losers_bracket_survivors(prova, round_number)
    ...
    def _generate_grand_final_pairings(self, prova: "Prova", round_number: int, 
        ...
        if len(active_players) == 1 and len(eliminated_once) == 1:
            return [Pairing(players=(active_players[0], eliminated_once[0]), round_number=round_number)]
    ...
    def _get_recent_winners_bracket_losers(self, prova: "Prova", round_number: int) -> List[int]:
        ...
        recent_matches = Match.query.filter_by(
            ...
            round_number=round_number - 1,
            status="completed"
        ).filter(Match.notes != "losers_bracket").all()
    ...
    def _get_previous_losers_bracket_survivors(self, prova: "Prova", round_number: int) -> List[int]:
        ...
        previous_losers_matches = Match.query.filter_by(
            prova_id=prova.id,
            round_number=round_number - 1,
            status="completed"
        ).filter(Match.notes == "losers_bracket").all()
