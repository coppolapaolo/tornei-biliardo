...
class AdvancedAmalfiStrategy(BaseStrategy):
    ...
    description = "Amalfi algorithm with X-replacement, trio matches, and enhanced anti-rematch"
    min_players = 2
    max_players = None
    supports_byes = True
    ...
    def validate(self, prova: object) -> ValidationResult:
        ...
        all_warnings = list(base_result.warnings) + strategy_validation.get('warnings', [])
        
        if all_errors:
            return ValidationResult.failure(all_errors, all_warnings)
        else:
            return ValidationResult.success(messages=list(base_result.messages) + all_warnings, warnings=all_warnings)
    ...
    def _validate_strategy_specific(self, prova: object) -> Dict[str, List[str]]:
        ...
        if self._options.allow_trio_matches and prova.without_x:
    ...
    def _generate_pairings(
        ...
        for pairing in base_pairings:
            if pairing.is_bye and self._options.allow_x_replacement:
    ...
    def _find_suitable_challenge(self, prova: Prova, player_id: int) -> Optional[Challenge]:
        ...
        try:
            ...
            suitable_challenges = ChallengeService.get_suitable_challenges_for_x_replacement(
                prova.tournament_id
            )
            
            # Filter by player's skill level or other criteria
            if suitable_challenges:
                return suitable_challenges[0]  # Return first suitable challenge
        except Exception:
    ...
    def _get_active_players(self, prova: Prova) -> List[int]:
        ...
        return [inscription.user_id for inscription in prova.inscriptions 
                if not inscription.is_withdrawn]
    ...
    def _apply_side_effects(
        ...
        for pairing in pairings:
            if "Challenge:" in (pairing.notes or ""):
                ...
                self._record_challenge_assignment(prova, pairing, round_number)
            elif "Individual match" in (pairing.notes or ""):
                # Record individual match intention
                self._record_individual_match_intention(prova, pairing, round_number)
