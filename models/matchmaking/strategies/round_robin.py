...
class RoundRobinStrategy(PairingStrategy):
        ...
        try:
            ...
            if player_count < 3:
                return ValidationResult(
                    ok=False,
                    messages=("Round Robin requires at least 3 players",)
                )
            
            if player_count > 16:
                return ValidationResult(
                    ok=False,
                    messages=("Round Robin with more than 16 players may be impractical",)
                )
    ...
    def _generate_round_robin_schedule(self, player_ids: List[int]) -> List[List[Tuple[int, ...]]]:
