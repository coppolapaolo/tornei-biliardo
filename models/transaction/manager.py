...
class TransactionManager:
    ...
    def transaction(
        ...
        try:
            ...
            logger.debug(f"Transaction {transaction_id} completed in {metrics.duration_ms:.2f}ms")
    ...
    def serializable_transaction(self):
        ...
        with self.transaction(isolation_level=TransactionIsolationLevel.SERIALIZABLE) as context:
            yield context
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        committed = len([m for m in self._metrics_history if m.status == TransactionStatus.COMMITTED])
        rolled_back = len([m for m in self._metrics_history if m.status == TransactionStatus.ROLLED_BACK])
        failed = len([m for m in self._metrics_history if m.status == TransactionStatus.FAILED])
        ...
        return {
            ...
            "rolled_back": rolled_back,
            "failed": failed,
            "success_rate_percent": round(committed / total * 100, 1) if total > 0 else 0,
...
def serializable(domain: Optional[str] = None):
    ...
    return transactional(isolation_level=TransactionIsolationLevel.SERIALIZABLE, domain=domain)
...
class DomainService:
    ...
    def domain_transaction(
        ...
        with transaction_manager.transaction(isolation_level=isolation_level, read_only=read_only) as context:
            self._track_domain_access()
            yield context