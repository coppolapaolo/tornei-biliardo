...
from models.individual_match.models import PlayerAvailability, MatchProposal, ProposalInvitation
...
class AvailabilityService:
    ...
    def set_player_availability(
        ...
        if existing:
            ...
            existing.preferred_days = json.dumps(preferred_days) if preferred_days else None
    ...
    def set_venue_availability(
        ...
        if existing:
            existing.is_available = is_available
            existing.available_days = json.dumps(available_days) if available_days else None
            existing.preferred_times = preferred_times
            existing.updated_at = datetime.utcnow()
        else:
    ...
    def get_available_players_at_venue(
        ...
        query = (
            ...
            .join(BilliardHall, UserLocationAvailability.billiard_hall_id == BilliardHall.id)
            .filter(
    ...
    def notify_players_of_availability(
        ...
        default_message = f"{requesting_user.username} è disponibile a giocare presso {location}"
    ...
    def create_availability_based_match_request(
        ...
        proposal = IndividualMatchService.create_match_proposal(
            ...
            title=f"Match presso {location}",
            description=message or "Richiesta di match basata su disponibilità",
            location=location,
            proposed_datetime=proposed_datetime,
            discipline=None,  # Optional
            distance=None,    # Optional
            break_rules=None, # Optional
            entry_fee=0.0
        )

        # Send invitation to target user
        IndividualMatchService.send_invitation(
