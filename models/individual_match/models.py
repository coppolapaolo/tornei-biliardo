...
class MatchProposal(BaseModel, TimestampMixin):
    ...
    proposer_id = db.Column(db.Integer, db.ForeignKey("user.id", ondelete="CASCADE"), nullable=False)
    
    # Proposal details
    proposal_type = db.Column(db.Enum(ProposalType), nullable=False)
    status = db.Column(db.Enum(ProposalStatus), nullable=False, default=ProposalStatus.PENDING)
    ...
    # Result tracking
    accepted_by_id = db.Column(db.Integer, db.ForeignKey("user.id", ondelete="SET NULL"), nullable=True)
...
class ProposalInvitation(BaseModel, TimestampMixin):
    ...
    id = db.Column(db.Integer, primary_key=True)
    proposal_id = db.Column(db.Integer, db.ForeignKey("match_proposal.id", ondelete="CASCADE"), nullable=False)
    invited_user_id = db.Column(db.Integer, db.ForeignKey("user.id", ondelete="CASCADE"), nullable=False)
...
class IndividualMatch(BaseModel, TimestampMixin):
    ...
    id = db.Column(db.Integer, primary_key=True)
    proposal_id = db.Column(db.Integer, db.ForeignKey("match_proposal.id"), nullable=True)
    
    # Players
    player1_id = db.Column(db.Integer, db.ForeignKey("user.id", ondelete="CASCADE"), nullable=False)
    player2_id = db.Column(db.Integer, db.ForeignKey("user.id", ondelete="CASCADE"), nullable=False)
    ...
    def add_rack_result(self, winner_id: int, rack_number: Optional[int] = None) -> 'IndividualRack':
        ...
        if rack_number is None:
            max_rack = db.session.query(func.max(IndividualRack.rack_number)).filter_by(match_id=self.id).scalar()
        ...
        elif not self.best_of and self.player1_score + self.player2_score >= self.distance:
...
class IndividualRack(BaseModel, TimestampMixin):
    ...
    id = db.Column(db.Integer, primary_key=True)
    match_id = db.Column(db.Integer, db.ForeignKey("individual_match.id", ondelete="CASCADE"), nullable=False)
...
class PlayerAvailability(BaseModel, TimestampMixin):
    ...
    preferred_days = db.Column(db.String(20), nullable=True)  # JSON array of weekday numbers
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'location', name='uq_user_location_availability'),
    )
