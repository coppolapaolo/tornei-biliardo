...
class PlayoffConfiguration(BaseModel, TimestampMixin):
    ...
    name = db.Column(db.String(100), nullable=False)  # e.g., "Elite Playoff", "Academy Playoff"
    playoff_type = db.Column(db.Enum(PlayoffType), nullable=False)
    description = db.Column(db.Text, nullable=True)
    
    # Qualification criteria
    max_participants = db.Column(db.Integer, nullable=False)
    min_provas_played = db.Column(db.Integer, nullable=True)  # Minimum provas to qualify
    qualification_criteria = db.Column(db.Text, nullable=False)  # JSON string with criteria
    ...
    auto_generate = db.Column(db.Boolean, nullable=False, default=True)  # Auto-generate at tournament end
    ...
    def evaluate_qualifications(self) -> List[Dict[str, Any]]:
        ...
        if self.playoff_type == PlayoffType.TOP_N:
            ...
            if criteria.get("category") == "elite":
                ...
                target_classifications = classifications[elite_positions:elite_positions + academy_positions]
    ...
    def _meets_minimum_requirements(self, user_id: int) -> bool:
        ...
        provas_played = (Inscription.query
                        .join(Prova)
                        .filter(
    ...
    def _evaluate_custom_criteria(self, classification: 'Classification', criteria: Dict[str, Any]) -> bool:
        ...
        if min_point_difference and classification.total_point_difference < min_point_difference:
            return False
...
class PlayoffQualification(BaseModel, TimestampMixin):
    ...
    id = db.Column(db.Integer, primary_key=True)
    configuration_id = db.Column(db.Integer, db.ForeignKey("playoff_configuration.id", ondelete="CASCADE"), nullable=False)
    ...
    # Status tracking
    status = db.Column(db.Enum(QualificationStatus), nullable=False, default=QualificationStatus.PENDING)
    notified_at = db.Column(db.DateTime, nullable=True)
    responded_at = db.Column(db.DateTime, nullable=True)
    
    # Replacement tracking
    replaced_by_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=True)
    replacement_position = db.Column(db.Integer, nullable=True)  # Position in replacement queue
    
    # Relationships
    configuration = db.relationship("PlayoffConfiguration", back_populates="qualifications")
...
class PlayoffTournament(BaseModel, TimestampMixin):
    ...
    id = db.Column(db.Integer, primary_key=True)
    configuration_id = db.Column(db.Integer, db.ForeignKey("playoff_configuration.id", ondelete="CASCADE"), nullable=False)
    prova_id = db.Column(db.Integer, db.ForeignKey("prova.id"), nullable=True)  # The actual playoff prova
    
    # Tournament details
    name = db.Column(db.String(100), nullable=False)
    status = db.Column(db.String(20), nullable=False, default="setup")  # setup, registration, playing, completed
    ...
    # Relationships
    configuration = db.relationship("PlayoffConfiguration", back_populates="playoff_tournament")
    ...
    def start_registration(self) -> None:
        ...
        confirmed_qualifications = (self.configuration.qualifications
                                   .filter_by(status=QualificationStatus.CONFIRMED)
                                   .all())
