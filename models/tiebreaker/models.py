...
class SpotShot(db.Model):
    ...
    # Relationships
    tiebreaker: Mapped["Tiebreaker"] = relationship("Tiebreaker", back_populates="spot_shots")
...
class RallyAttempt(db.Model):
    ...
    # Relationships
    tiebreaker: Mapped["Tiebreaker"] = relationship("Tiebreaker", back_populates="rally_attempts")
    player: Mapped["User"] = relationship("User")
    
    def __repr__(self):
        return f"<RallyAttempt {self.id} {self.player.username} {self.points_scored}pts>"
...
class PlayoffMatch(db.Model):
    ...
    # Relationships
    tiebreaker: Mapped["Tiebreaker"] = relationship("Tiebreaker", back_populates="playoff_matches")
    player1: Mapped["User"] = relationship("User", foreign_keys=[player1_id])
    player2: Mapped["User"] = relationship("User", foreign_keys=[player2_id])
    winner: Mapped[Optional["User"]] = relationship("User", foreign_keys=[winner_id])
