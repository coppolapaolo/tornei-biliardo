...
class User(UserMixin, BaseModel):
    ...
    role = db.Column(
        db.String(20), nullable=False, default="player", index=True
    ...
    tournament_director_associations = db.relationship(
        "TournamentDirector",
        foreign_keys="TournamentDirector.user_id",
        backref="director",
    )
    ...
    def can_manage_tournament(self, tournament_id):
        ...
        if self.is_director:
            ...
            assignment = TournamentDirector.query.filter_by(
                user_id=self.id, tournament_id=tournament_id
            ).first()
            ...
            tournaments_played = len(
                set(
                    [
                        ...
                        for insc in Inscription.query.filter_by(user_id=self.id)
                        .join(Prova)
                        .all()
                    ]
                )
    ...
    def validate(self):
        ...
        existing_username = User.query.filter(
            User.username == self.username, User.id != self.id if self.id else True
        ).first()
        if existing_username:
            errors.append("Username already exists")

        existing_email = User.query.filter(
            User.email == self.email, User.id != self.id if self.id else True
        ).first()
        if existing_email:
...
class TournamentDirector(BaseModel):
    ...
    def __repr__(self):
        return (
            f"<TournamentDirector user_id={self.user_id} "
