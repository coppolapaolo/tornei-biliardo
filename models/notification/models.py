...
class NotificationType(Enum):
    ...
    TOURNAMENT_REGISTRATION = "tournament_registration"  # Tournament registration opened
...
class Notification(BaseModel, TimestampMixin):
    ...
    def dismiss(self) -> None:
        ...
        if self.status in [NotificationStatus.SENT, NotificationStatus.PENDING, NotificationStatus.READ]:
...
class NotificationPreference(BaseModel, TimestampMixin):
    ...
    email_enabled = db.Column(db.Boolean, nullable=False, default=False)  # Future: email notifications
    push_enabled = db.Column(db.Boolean, nullable=False, default=True)   # Future: push notifications
    ...
    # Frequency limits
    max_per_day = db.Column(db.Integer, nullable=True)     # Max notifications per day for this type
    min_interval_minutes = db.Column(db.Integer, nullable=True)  # Minimum interval between notifications
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'notification_type', name='uq_user_notification_preference'),
    )
    
    @classmethod
    def get_user_preference(cls, user_id: int, notification_type: NotificationType) -> Optional['NotificationPreference']:
    ...
    @classmethod
    def is_notification_enabled(cls, user_id: int, notification_type: NotificationType) -> bool:
    ...
    def can_send_notification(self) -> bool:
        ...
        return (self.enabled and 
                not self.is_in_quiet_hours() and 
                not self.has_reached_daily_limit() and 
                not self.has_violated_interval())
...
class NotificationTemplate(BaseModel, TimestampMixin):
    ...
    # Default settings
    default_priority = db.Column(db.Enum(NotificationPriority), nullable=False, default=NotificationPriority.NORMAL)
    default_expires_hours = db.Column(db.Integer, nullable=True)  # Hours until expiration
    ...
    def render_notification(self, context: Dict[str, Any]) -> Dict[str, Optional[str]]:
        ...
        return {
            ...
            "action_text": replace_vars(self.action_text_template) if self.action_text_template else None,
            "action_url": replace_vars(self.action_url_template) if self.action_url_template else None
        }
