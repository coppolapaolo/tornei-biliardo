...
def challenge_detail(challenge_id):
    """View challenge details and user's attempts."""
    try:
        challenge = Challenge.query.get_or_404(challenge_id)
        user_best_attempt = challenge.get_user_best_attempt(current_user.id)
        
        # Get user's attempts for this challenge
        user_attempts = ChallengeAttempt.query.filter_by(
        ...
        flash(f"Error loading challenge: {str(e)}", "danger")
...
def attempt_detail(attempt_id):
    """View attempt details (for taking the challenge)."""
    try:
        attempt = ChallengeAttempt.query.get_or_404(attempt_id)
        
        # Verify user owns this attempt
        if attempt.user_id != current_user.id:
            flash("Access denied to this challenge attempt.", "danger")
            return redirect(url_for('challenge.challenge_catalog'))
        
        return render_template('challenge/attempt.html', attempt=attempt)
        
    except Exception as e:
        flash(f"Error loading attempt: {str(e)}", "danger")
        return redirect(url_for('challenge.challenge_catalog'))
...
def complete_attempt(attempt_id):
    ...
    try:
        ...
        if request.is_json:
            ...
            return redirect(url_for('challenge.challenge_detail', 
...
def challenge_statistics(challenge_id):
    ...
    try:
        ...
        if request.is_json:
            ...
            return render_template('challenge/statistics.html',
                                 challenge=challenge,
                                 statistics=statistics)
...
@challenge_bp.route('/x-replacement/<int:prova_id>/<int:round_number>', methods=['POST'])
@login_required
