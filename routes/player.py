...
def dashboard():
    ...
    if selected_tournament_id:
        selected_tournament = Tournament.query.filter_by(
            id=selected_tournament_id, is_active=True
        ).first()
    ...
    # Filtra quelle dove l'utente non è già iscritto
    standalone_available = [
        p for p in standalone_available if not p.is_user_inscribed(current_user.id)
    ]

    # Iscrizioni a prove standalone
    my_standalone_inscriptions = (
        Inscription.query.join(Prova)
        .filter(Inscription.user_id == current_user.id, Prova.tournament_id.is_(None))
        .all()
    )
    ...
    return render_template(
        ...
        my_standalone_inscriptions=my_standalone_inscriptions,
    )
