...
from models.individual_match.models import (
    MatchProposal, IndividualMatch, ProposalType, ProposalStatus, MatchStatus
)
...
def create_proposal():
    ...
    try:
        ...
        proposal_data = {
            ...
            'invited_user_ids': data.getlist('invited_user_ids') if data.get('invited_user_ids') else []
        }
        ...
        if request.is_json:
            ...
            return redirect(url_for('individual_match.proposal_detail', proposal_id=proposal.id))
...
def proposal_detail(proposal_id):
    ...
    try:
        ...
        can_view = (
            proposal.proposer_id == current_user.id or
            proposal.proposal_type == ProposalType.OPEN or
            any(inv.invited_user_id == current_user.id for inv in proposal.invitations)
        )
        ...
        can_accept = proposal.can_be_accepted_by(current_user.id)
        
        return render_template('individual_match/proposal_detail.html',
                             proposal=proposal,
...
def accept_proposal(proposal_id):
    ...
    try:
        individual_match = MatchProposalService.accept_proposal(proposal_id, current_user.id)
        ...
        if request.is_json:
            ...
            return redirect(url_for('individual_match.match_detail', match_id=individual_match.id))
        ...
        if request.is_json:
            ...
            flash(error_msg, "danger")
            return redirect(url_for('individual_match.proposal_detail', proposal_id=proposal_id))
...
def manage_availability():
    ...
    if request.method == 'GET':
        availability_data = IndividualMatchService.get_user_availability(current_user.id)
        return render_template('individual_match/availability.html', **availability_data)
